...
public:
	...
	*/
	...
	bool is_wall(int parallel_wall[16][17], int vertical_wall[17][16], pair<int, int> pos,int action){
        ...
        y = pos.second / 25;
		if((pos.first - 1) % 25 != 0){
			x += abs(dir.first)/dir.first;
		}
		if((pos.second - 1) % 25 != 0){
			y += abs(dir.second)/dir.second;
		}
	...
	int dfs(pair<int, int> cur_pos, int ghostStat[4][2], std::vector<std::vector<int>> propsStat, int parallel_wall[16][17], int vertical_wall[17][16], vector<pair<int, int>> visited, int reward, int depth){
		...
		int adddir[2];
		adddir[0] = cur_pos.first ;
		adddir[1] = cur_pos.second ;

		int t_x, t_y;
		t_x = (cur_pos.first + dir.first)/25;
		t_y = (cur_pos.second + dir.second)/25;

		// if(depth == 8){
		// 	if( (t_x != cur_pos.first/25) && t_x <= 15 && t_x >= 0){
		// 		adddir[0] += (t_x%25 + 1);
		// 	}
		// 	else if( (t_y != cur_pos.second/25) && t_y <= 15 && t_y >= 0){	
		// 		adddir[1] += (t_y%25 + 1);
		// 	}
		// }

		pair<int, int> pos_cord = make_pair(adddir[0]/25, adddir[1]/25);
		...
		for(int i = 0; i < propsStat.size(); i++){
			...
			// 	reward -= 50*depth;
			// }
			else if((abs(cur_pos.first - propsStat[i][1] ) + abs(cur_pos.second - propsStat[i][2])) <= 15 && propsStat[i][0] == 3){
				reward -= 200*depth;
			}
	...
	int no_p(pair<int, int> cur_pos  , std::vector<std::vector<int>> propsStat){
		...
		for(int i = 0; i < propsStat.size();i++){
			if(propsStat[i][0] == 0){
				score += (40 - (abs(x - (propsStat[i][1]/25)) + abs(y - propsStat[i][2]/25)));
			}
			else if(propsStat[i][0] == 1){
				score += (100 - (abs(x - (propsStat[i][1]/25)) + abs(y - propsStat[i][2]/25)));
			}
			else if(propsStat[i][0] == 2){
				score += (50 - (abs(x - (propsStat[i][1]/25)) + abs(y - propsStat[i][2]/25)));
			}
			else if(propsStat[i][0] == 3){
				score -= ( 50 + (abs(x - (propsStat[i][1]/25)) + abs(y - propsStat[i][2]/25)));
			}
		}
	...
	void GetStep(int playerStat[4], int ghostStat[4][2], std::vector<std::vector<int>> propsStat,int parallel_wall[16][17], int vertical_wall[17][16])
	{
		...
		pair<int, int> pos;
		if(playerStat[0] - prev_pos.first != 0 || playerStat[1] - prev_pos.second != 0){
			dir = make_pair(playerStat[0] - prev_pos.first, playerStat[1] - prev_pos.second);
		}
		...
		pair<int, int> cur_pos = make_pair(tx, ty);
		...
		for (int i = 0; i < 4;i++){
			...
			if(is_valid(parallel_wall, vertical_wall, cur_pos, i, dir)){
				...
				reward += dfs(pos, ghostStat, propsStat, parallel_wall, vertical_wall, visited, 0, 8);
				if(reward == 0) flag++;
				if(pstat <= 0){
					reward -= gdfs(pos, ghostStat, propsStat, parallel_wall, vertical_wall, gvisited, 0, 3);
				}
