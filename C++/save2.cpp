...
public:
	...
	*/
	...
	bool is_centered(pair<int, int> pos, pair<int, int> dir){
		...
		// if(pstat <= 0){
			if((x_cord / 25 != (x_cord + dir.first) / 25) || (y_cord / 25 != (y_cord + dir.second) / 25)){
				return true;
			}
			else if(x_cord % 25 == 0 && y_cord % 25 == 0){
				return true;
			}
			else return false;
	...
	int dfs(pair<int, int> cur_pos, int ghostStat[4][2], std::vector<std::vector<int>> propsStat, int parallel_wall[16][17], int vertical_wall[17][16], vector<pair<int, int>> visited, int reward, int depth){
		...
		for(int i = 0; i < propsStat.size(); i++){
			...
			// }
			// if(abs(cur_pos.first -propsStat[i][1] ) + abs(cur_pos.second -propsStat[i][2] ) <= 12 ){
			// 	reward -= 50*depth;
			// }

			if(pos_cord == make_pair((propsStat[i][1])/25, (propsStat[i][2])/25) && propsStat[i][0] == 3){
				reward -= 50*depth;
			}
			//pellet
			else if(pos_cord == make_pair(propsStat[i][1]/25, propsStat[i][2]/25) && propsStat[i][0] == 2){
				reward += 10*depth;
			}
			//fruit
	...
	int no_p(pair<int, int> cur_pos  , std::vector<std::vector<int>> propsStat){
		...
		for(int i = 0; i < propsStat.size();i++){
			score += 30 - (abs(x - (propsStat[i][0]/25)) + abs(y - propsStat[i][1]/25));
		}
		return score;
	}
